...
return; 
 ...
 <tr >
     <h1 style="color:red">
     <td><input type="radio" name="choice"  onclick="document.getElementById('delete').removeAttribute('disabled');" value="<%=rs.getInt("id")%>"><%=rs.getInt("id")%></input></td>
   	   	<td><%=rs.getString("empName")%></td>
   	   		<td><%=rs.getString("custName")%></td>
   	   	<td><%=rs.getString("prodName")%></td>
   		 <td><%=rs.getString("prodType")%></td>
   		 <td><%=rs.getString("company")%>
   		  <td><%= rs.getFloat("prodcost")%></td>
   		 <td><%=rs.getInt("quantity")%></td>
   		 <td><%=rs.getFloat("prodamount") %></td>
   		 <td><%=rs.getFloat("prodtax") %></td>
   		 <td><%=rs.getFloat("prodgross") %></td>
   		 <td><%=rs.getFloat("proddiscount") %></td>
   		 <td><%=rs.getFloat("prodgrand") %></td>
   		 </h1>
   		</tr>
  </table>
 <%
