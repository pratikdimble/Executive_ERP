...
<title>Select Incentive Record</title>
<body  >
...
<c:choose>
  <c:when test="${!empty listincentiveDTOs}">
   <tr>
    ...
    <th>Target Per Product</th>
    <th>Incentive Per Product(Rs.)</th>
    ...
    <c:forEach var="dto"  items="${listincentiveDTOs}">
     ...
     <tr>
     <h1 style="color:red">
   		 <td><input type="radio" name="choice" value="${dto.incId}">${dto.incId}</input></td>
   		 <td>${dto.incDate}</td>
   		 <td>${dto.empName}</td>
   		 <td>${dto.prodName}</td>
   		 <td>${dto.prodType}</td>
		 ...
		 <td>${dto.achieved}</td>
   		 <td>${dto.workdone}</td>
   		 <td>${dto.amount}</td>
		 <td><button type="submit" name="operation" value="DELETE" onClick="return conf()">DELETE</button></td>
   		 </h1>
   		</tr>
    </c:forEach>
  </c:when>
