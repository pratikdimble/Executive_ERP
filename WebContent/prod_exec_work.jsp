...
<title>Product Executive:: To Do List</title>
<body  >
...
Connection con= null;
	...
	String driverName = "oracle.jdbc.OracleDriver";
		...
		String assign_sql = "SELECT * FROM ERP_EMPASSIGN  Where DESIGN='Product Executive' and  EMPNAME=?";
  ...
  <div class="header-right">
   ...
   <th>Area Name</th>
     ...
     <%
		while (rs2.next()) {
	%>
	<td><label value="<%=rs2.getInt("ID")%>"><%=rs2.getInt("ID")%></label></td>
	<td><label value="<%=rs2.getString("ASSIGNDATE")%>"><%=rs2.getString("ASSIGNDATE")%></label></td>
	<td><label value="<%=rs2.getString("PRODNAME")%>"><%=rs2.getString("PRODNAME")%></label></td>
	<td><label value="<%=rs2.getString("PRODTYPE")%>"><%=rs2.getString("PRODTYPE")%></label></td>
